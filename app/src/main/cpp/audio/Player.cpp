...
Player::Player(std::shared_ptr<DataSource> source) {
    mSource = source;
}
...
//        LOGE("ss%f", mVolumeValue);

        for (int i = 0; i < framesToRenderFromData; ++i) {
            for (int j = 0; j < properties.channelCount; ++j) {
                targetData[(i * properties.channelCount) + j] =
                        data[(mReadFrameIndex * properties.channelCount) + j] * mVolumeValue;
            }
